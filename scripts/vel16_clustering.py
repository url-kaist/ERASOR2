...
def concentric_zone_based_ground_segmentation(points, range_thrs, height_thrs):
    """Segment ground points using concentric zone-based approach."""
    ...
    for i in range(num_thrs):
        if i == 0:
            ...
            range_within = np.bitwise_and(
                range_thrs[i - 1] < range_wrt_origin, range_wrt_origin < range_thrs[i]
            )
...
if __name__ == "__main__":
    ...
    abs_ground_dir = (
        "/media/shapelim/UX980/erasor_inputs/bongeunsa_dataset/"
        "ground_labels_via_patchwork2"
    )
    abs_instance_dir = (
        "/media/shapelim/UX980/erasor_inputs/bongeunsa_dataset/"
        "instance_labels_via_patchwork2"
    )

    for i in range(0, 1320):
        zfilled_idx = str(i).zfill(6)
        pcd_path = (
            "/media/shapelim/UX980/erasor_inputs/bongeunsa_dataset/pcds/"
            + zfilled_idx
            + ".pcd"
        )
        ...
        if use_pre_extracted_ground_label:
            ground_file = (
                f"/media/shapelim/UX980/erasor_inputs/bongeunsa_dataset/"
                f"ground_labels_via_patchwork2/{zfilled_idx}.label"
            )
            ...
            ground_inliers = concentric_zone_based_ground_segmentation(
                pcd.points, [4.0, 10.0, float("inf")], [-1.35, -1.2, -1.0]
            )
