...
def read_pcd_o3d_tensor(path):
    ...
    # Convert intensity to uint32 (either via value casting or bit reinterpretation)
    intens_u32 = to_uint32_intensity(intens)
    return xyz, intens_u32

def to_uint32_intensity(arr):
    """
    Convert intensity array to uint32, handling both numeric and bit-packed cases.
    - If dtype is already uint32, return as is.
    - If dtype is float and values are close to integers, cast directly.
    - If dtype is float with bit-encoded values, reinterpret bits as uint32 (float32 only).
    """
    a = np.asarray(arr)
...
def fetch_dynamic_objects_ids(intensity_u32):
    """
    Print unique instance IDs for each dynamic class in the given intensity array.
    """
...
def count_static_and_dynamic(intensity_u32, verbose=False):
    ...
    NUM_CLASS_ON_MAP = {252: 0, 253: 0, 254: 0, 255: 0,
                        ...
                        'dynamic': 0, 'static': 0, 'percentage': 0.0, 'total': 0}
    ...
    if verbose:
        print(f"# Total - {ns+nd} => Static : {ns} | Dynamic: {nd} | percentage: {NUM_CLASS_ON_MAP['percentage']:.3f}")
    return NUM_CLASS_ON_MAP

# ---------- metrics ----------
def calc_naive_preservation(gt_xyz, gt_int_u32, est_xyz, est_int_u32, dists, indices, voxelsize):
    """
    ...
    for gt_sem, est_sem in tqdm(zip(gt_inliers, est_inliers), total=len(gt_inliers), desc="Preservation"):
...
def evaluate(gt_xyz, gt_int_u32, est_xyz, est_int_u32, voxelsize=0.2):
    ...
    num_preserved, num_static_preserved, num_dynamic_preserved = \
        calc_naive_preservation(gt_xyz, gt_int_u32, est_xyz, est_int_u32, dists, indices, voxelsize)

    gt_data  = [num_gt['static'], num_gt['dynamic'], f"{num_gt['percentage']:.3f}"]
    est_data = [num_estimate['static'], num_estimate['dynamic'], f"{num_estimate['percentage']:.3f}"]

    precision = float(num_gt['dynamic'] - num_estimate['dynamic']) / float(num_gt['total'] - num_estimate['total'] + 1e-11)
    recall    = float(num_gt['dynamic'] - num_estimate['dynamic']) / float(num_gt['dynamic'] + 1e-11)

    pr = float(num_static_preserved) / float(max(num_gt['static'], 1)) * 100.0
    rr = float(num_gt['dynamic'] - num_dynamic_preserved) / float(max(num_gt['dynamic'], 1)) * 100.0
    f1 = 0.0
    if (pr + rr) > 0:
...
def main():
    ...
    parser.add_argument('--gt',  required=True, type=str, help="Path to ground truth PCD")
    parser.add_argument('--est', required=True, type=str, help="Path to estimated PCD")
    parser.add_argument('--vox', default=0.2, type=float, help="Voxel size for distance threshold")
