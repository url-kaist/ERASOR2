...
int main(int argc, char **argv) {
    ...
    for (int i   = start_frame; i < end_frame + accum_interval; ++i) {
        ...
        pcl::PointCloud<pcl::PointXYZI>::Ptr est_static_transformed(new pcl::PointCloud<pcl::PointXYZI>);
        ...
        pcl::transformPointCloud(*est_static, *est_static_transformed, pose * tf_h_of_ground_to_be_zero);
        (*static_map_accum) += (*est_static_transformed);
    }

    erasor_utils::voxelize_preserving_labels_by_nanoflann(static_map_accum, *static_map_voxelized, params->voxel_size_);
