...
PointCloudProcessor::PointCloudProcessor(std::string absPath,
  ...
  if (trajectoryDir.back() == '/') {
    trajectoryDir.pop_back();
  }
  // displayBanner();
  // gatherInput();
  ...
  if (!std::filesystem::exists(saveDir)) {
    std::cout << yellow << "The path does not exist. Creating a new directory." << reset
              << std::endl;
...
// Clear the screen
  ...
  const std::string description =
      "This utility facilitates the undistortion of .bin point cloud files, "
  ...
  const std::string inputInfo =
      "Input: .bin files, trajectory file\n"
      "Output: .pcd files\n"
      "1. Bin files from HeLiPR should be needed.\n"
      "2. trajectory file is tum form made by transformINStoLiDAR.py.";

  const std::string maintainerInfo =
      "Maintainer: Minwoo Jung (moonshot@snu.ac.kr, SNU RPM Lab), Revised: 2023/12/06";
...
void PointCloudProcessor::gatherInput() {
  absPath = visualizer.getInput(
      cyan +
      "Enter the path to the directory (ex) /media/se0yeon00/SY_Other/HeliPR/KAIST05: " + reset);
  sensorType = visualizer.getInput(
      cyan + "Enter the sensor type, (ex) Aeva, Avia, Ouster, Velodyne: " + reset);
  ...
  while (!std::filesystem::exists(trajPath) || trajPath.substr(trajPath.size() - 4) != ".txt") {
    std::cout << red << "The path does not exist or the file is not txt. Please try again." << reset
              << std::endl;
    trajPath = visualizer.getInput(cyan + "Enter the path to the trajectory file: " + reset);
  }
  ...
  while (!pathCreated) {
    saveDir =
        visualizer.getInput(cyan +
                            "Enter the path to the directory to save the processed point clouds "
                            "(ex) /media/se0yeon00/SY_Other/HeliPR/KAIST05/deskewed_LiDAR: " +
                            reset);
    ...
    if (savePath.back() != '/') {
      ...
      if (!std::filesystem::exists(savePath)) {
        ...
        if (!std::filesystem::create_directory(savePath)) {
          std::cout << red << "Failed to create the directory. Please try again." << reset
                    << std::endl;
        } else {
  ...
  int lidarInput = visualizer.getInput<int>(
      cyan + "Enter the LiDAR type (0: Ouster, 1: Velodyne, 2: Livox, 3: Aeva): " + reset, 0);
  ...
  while (LiDAR < 0 || LiDAR > 3) {
    ...
    lidarInput = visualizer.getInput<int>(
        cyan + "Enter the LiDAR type (0: Ouster, 1: Velodyne, 2: Livox, 3: Aeva): " + reset, 0);
    LiDAR = static_cast<LiDARType>(lidarInput);
  }

  distanceThreshold = visualizer.getInput<float>(
      cyan + "Enter the distance threshold (m) (default: 10, >= 0): " + reset, 10.0f);
  ...
  numIntervals = visualizer.getInput<int>(
      cyan + "Enter the number of intervals for interpolation (default: 1000, >= 1): " + reset,
      1000);
  ...
  accumulatedSize = visualizer.getInput<int>(
      cyan +
          "Enter the number of point clouds to accumulate before processing (default: 20, >= 1): " +
          reset,
      20);
  while (accumulatedSize < 1) {
    std::cout << red << "Invalid accumulated size. Please try again." << reset << std::endl;
    accumulatedSize = visualizer.getInput<int>(cyan +
                                                   "Enter the number of point clouds to accumulate "
                                                   "before processing (default: 20, >= 1): " +
                                                   reset,
                                               20);
  }

  downSampleFlag = visualizer.getInput<bool>(
      cyan + "Enter the downsample flag (0: false, 1: true) (default: 1): " + reset, true);
  ...
  if (downSampleFlag) {
    downSampleSize = visualizer.getInput<float>(
        cyan + "Enter the downsample size (m) (default: 0.4): " + reset, 0.4f);
  ...
  undistortFlag = visualizer.getInput<bool>(
      cyan + "Enter the undistort flag (0: false, 1: true) (default: 1): " + reset, true);
...
void PointCloudProcessor::displayInput() {
  ...
  std::cout << green << std::string(40, '-') << reset << std::endl;  // Divider line
  std::cout << green << std::left << std::setw(width) << "binPath:" << binPath << reset
            << std::endl;
  std::cout << green << std::left << std::setw(width) << "trajPath:" << trajPath << reset
            << std::endl;
  std::cout << green << std::left << std::setw(width) << "savePath:" << savePath << reset
            << std::endl;
  std::string lidarTypeStr = visualizer.lidarTypeToString(LiDAR);
  std::cout << green << std::left << std::setw(width) << "LiDAR:" << lidarTypeStr << reset
            << std::endl;
  std::cout << green << std::left << std::setw(width) << "Distance Threshold:" << distanceThreshold
            << reset << std::endl;
  std::cout << green << std::left << std::setw(width) << "Num Intervals:" << numIntervals << reset
            << std::endl;
  std::cout << green << std::left << std::setw(width) << "Accumulated Size:" << accumulatedSize
            << reset << std::endl;
  std::cout << green << std::left << std::setw(width)
            << "Downsample Flag:" << (downSampleFlag ? "True" : "False") << reset << std::endl;
  if (downSampleFlag)
    std::cout << green << std::left << std::setw(width) << "Downsample Size:" << downSampleSize
              << reset << std::endl;
  std::cout << green << std::left << std::setw(width)
            << "Undistort Flag:" << (undistortFlag ? "True" : "False") << reset << std::endl;
  std::cout << std::endl;
}

std::pair<long long, std::vector<float>> PointCloudProcessor::splitLine(std::string input,
                                                                        char delimiter) {
...
void PointCloudProcessor::loadAllPoses(const std::string &pose_path,
...
void PointCloudProcessor::vec2tf4x4(const std::vector<float> &pose, Eigen::Matrix4f &tf4x4) {
  ...
  tf4x4 << mat3(0, 0), mat3(0, 1), mat3(0, 2), pose[0], mat3(1, 0), mat3(1, 1), mat3(1, 2), pose[1],
      mat3(2, 0), mat3(2, 1), mat3(2, 2), pose[2], 0, 0, 0, 1;
}

void PointCloudProcessor::interpolate(double timestamp,
                                      double timeStart,
                                      double dt,
  ...
  if (idx < 0) idx = 0;
  if (idx >= numIntervals - 1) idx = numIntervals - 2;
...
template <class T>
void PointCloudProcessor::processPoint(T &point,
                                       double timestamp,
                                       double timeStart,
                                       double dt,
...
void PointCloudProcessor::readBinFile(const std::string &filename,
...
template <class T>
void PointCloudProcessor::accumulateScans(std::vector<pcl::PointCloud<T>> &vecCloud,
                                          Point3D &lastPoint) {
  ...
  if (vecCloud.size() == accumulatedSize) {
    if (euclidean_distance(lastPoint, scanPoints[0]) > distanceThreshold) {
      visualizer.progressBar(
          keyIndex, numBins, padZeros(keyIndex - accumulatedSize, 6) + "." + saveFormat, true);
      ...
      for (int i = 0; i < accumulatedSize; i++) {
        ...
        for (; pclIter != vecCloud[i].points.begin(); pclIter--) {
          ...
          transformedPoint = scanQuat[0].conjugate() *
                             ((scanQuat[i] * transformedPoint + scanTrans[i]) - scanTrans[0]);
          point.x = transformedPoint(0);
          point.y = transformedPoint(1);
          point.z = transformedPoint(2);
      ...
      if (downSampleFlag) {
        ...
        if (saveFormat == "pcd") {
          pcdWriter.writeBinary(savePath + padZeros(keyIndex - accumulatedSize, 6) + ".pcd",
                                *sampledCloud);
      ...
      fout_pose << tf4x4(0, 0) << " " << tf4x4(0, 1) << " " << tf4x4(0, 2) << " " << tf4x4(0, 3)
                << " " << tf4x4(1, 0) << " " << tf4x4(1, 1) << " " << tf4x4(1, 2) << " "
                << tf4x4(1, 3) << " " << tf4x4(2, 0) << " " << tf4x4(2, 1) << " " << tf4x4(2, 2)
                << " " << tf4x4(2, 3) << std::endl;
      ++numSaved;
    }
...
void PointCloudProcessor::processFile(
    const std::string &filename)  // 여기서의 filename 은 LiDAR 파일의 파일명을 의미
{
  ...
  std::vector<Eigen::Quaterniond> quaternions(
      numIntervals);  // 인터벌로 지정해준 수만큼의 쿼터니온 및 position 을 가져와준다. 여기서 나는
                      // 1000으로 지정했다.
  ...
  if (timeStart < trajPoints[0](0) - 100 ||
      timeStart > trajPoints[trajPoints.size() - 1](0) + 100) {
    std::cout << red
              << "The timestamp is out of range. Please check the trajectory file and bin files."
              << reset << std::endl;
    exit(0);
  }

  bool success_field =
      bsplineSE3.get_pose(timeStart, qStart, pStart);  // 시작 orientation 및 시작 position 을
  // bSplineSE3 에는 trajectory 의 정보가 들어가있고, 이를 이용해서, 현재의 시간에 대한 quaternion
  // 과 position 을 가져온다.
  ...
  if (success_field) {
    ...
    double dt      = (timeEnd - timeStart) /
                numIntervals;  // 하나의 스캔이 들어오기 까지의 시간을 잘 쪼개서 (numIntervals 가 한
                               // 스캔을 디스큐잉할 레졸루션 같은 거였구나...)
    bool exitFlag = false;
...
#pragma omp parallel for
          ...
          //   processPoint(point, timestamp, timeStart, dt, qStart, pStart, quaternions, positions,
          //   numIntervals, qScan, pScan);
          // }
        }
...
void PointCloudProcessor::loadBinFileNames() {
  ...
  if ((dir = opendir(binPath.c_str())) != NULL) {
    while ((ent = readdir(dir)) != NULL) {
      ...
      if (filename.size() > 4 &&
          filename.substr(filename.size() - 4) == ".bin")  // binFiles 에 타임스탬프만 저장됨
      {
...
void PointCloudProcessor::ProcessBinFiles() {
  ...
  if (numSaved != numBins) {
    throw runtime_error(
        "\033[1;31mNumber of saved scans does not match number of bins. Please check it.\033[0m");
  }
}
...
void PointCloudProcessor::loadTrajectory() {
  ...
  while (std::getline(file, line)) {
    ...
    Eigen::VectorXd point(
        8);  // trajectory 가 찍힌 sec, 그 때의 포지션 x, y, z, ox, oy, oz, ow 순으로 들어감
    if (!(iss >> point(0) >> point(1) >> point(2) >> point(3) >> point(4) >> point(5) >> point(6) >>
          point(7))) {
      break;
    }
...
void PointCloudProcessor::getTransformedCloud(const int i,
