...
def latlon2local(lat, lon, alt, origin):
    """Convert latitude/longitude coordinates to local coordinates."""
...
def processTrajectory(inputPath, outputPath, typeLiDAR, visualize=False):
    """Process trajectory data and transform to LiDAR frame."""
    # Read the CSV file
    ...
    x, y, z = latlon2local(
        ...
        latlonheight[:, 2],
        latlonheight[:, 3],
        [latlonheight[0, 1], latlonheight[0, 2], latlonheight[0, 3]],
    )
    ...
    )  # 정의된 매트릭스를 transformation 시켜둔거네
...
if __name__ == "__main__":
    ...
    for lidar in process_lidar_list:
        ...
        outputTXT = os.path.join(
            save_ins_to_LiDAR_root, "{}_trajectory.txt".format(lidar)
        )  # input('Enter the path of the output file: ')
        typeLiDAR = (
            lidar  # input("Enter the LiDAR type (Livox, Aeva, Ouster, Velodyne): ")
        )
