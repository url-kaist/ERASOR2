...
void ERASOR::viz_pseudo_occupancy() {
    ...
    for (int theta = 0; theta < num_sectors; theta++) {
        for (int r = 0; r < num_rings; r++) {
            ...
            if (bin_curr.is_occupied && bin_map.is_occupied) {
                ...
                os << "(" << round(map_h_diff * 100) / 100.0 << ", " << round(curr_h_diff * 100) / 100.0 << ")"; // => " << round(scan_ratio * 100) / 100.0;
                ...
                marker.pose.position.x    = bin_map.x;
                marker.pose.position.y    = bin_map.y;
    ...
    std::cout << marker_arr.markers.size() << std::endl;
    pub_h_marker.publish(marker_arr);
