...
inline size_t DataLoader::size() const { return num_frames_; }
...
inline void DataLoader::vec2tf4x4(vector<float> &pose, Eigen::Matrix4f &tf4x4) {
  ...
  {
    Eigen::Matrix3f mat3 =
        Eigen::Quaternionf(pose[7], pose[4], pose[5], pose[6]).toRotationMatrix();
    tf4x4 << mat3(0, 0), mat3(0, 1), mat3(0, 2), pose[1], mat3(1, 0), mat3(1, 1), mat3(1, 2),
        pose[2], mat3(2, 0), mat3(2, 1), mat3(2, 2), pose[3], 0, 0, 0, 1;
  }
}
...
inline void DataLoader::getPose(const size_t i, Eigen::Matrix4f &pose) { pose = poses_gt_[i]; }

SemanticKITTILoader::SemanticKITTILoader(const string &abs_data_dir,
                                         const string &seq,
...
void SemanticKITTILoader::getScanAndPose(size_t i,
  ...
  assignLabels(*ground_label, *instance_label, cloud, max_instance);

  getPose(i, pose);
}

void SemanticKITTILoader::loadEstGroundAndInstanceLabels(const int i,
  ...
  if (instance_label.size() != ground_label.size()) {
    throw invalid_argument(
        "[Loading] Something's wrong! Instance and ground label sizes don't match!");
  }
...
void SemanticKITTILoader::assignLabels(const std::vector<uint32_t> ground_labels,
...
HeLiPRLoader::HeLiPRLoader(const string &abs_data_dir,
                           const string &sensor,
                           const string &instance_seg_method) {
  seq_                      = sensor;
  std::string abs_data_dir_ = abs_data_dir;
  if (abs_data_dir_.back() == '/') {
    abs_data_dir_.pop_back();
  }
  ...
  T_OS2_Avia << 0.999821044774776, 0.0159071210418969, 0.0102392346214582, 0.628232128507671,
      ...
      -0.00995801301399956, -0.0176587143630322, 0.999794482773264, -0.229651932062428, 0, 0, 0, 1;
  ...
  T_OS2_Aeva << 0.999566194824045, 0.0248733202636163, -0.0157714965695182, 0.541864230595932,
      ...
      0.0155950934721411, 0.00727017869078270, 0.999851957822456, -0.0869161258617896, 0, 0, 0, 1;
  ...
  T_OS2_VLP16 << 0.999752289753613, -0.0215240513828855, 0.00566342162254459, -0.958469101466382,
      ...
      -0.00554223034012042, 0.00568599350836049, 0.999968476083461, -0.262799643355476, 0, 0, 0, 1;
  // T_OS2_VLP16 = Eigen::Matrix4f::Identity();

  cout << ANSI_COLOR_GREEN << " [DataLoader] Data directories are as follows:" << ANSI_COLOR_RESET
       << endl;
...
inline void HeLiPRLoader::countNumFrames(const string &pcd_dir,
...
void HeLiPRLoader::getScanAndPose(size_t i,
                                  pcl::PointCloud<pcl::PointXYZI> &cloud,
                                  Eigen::Matrix4f &pose) {
...
void HeLiPRLoader::loadEstGroundAndInstanceLabels(const int i,
                                                  std::vector<uint32_t> &ground_label,
                                                  std::vector<uint32_t> &instance_label) {
  string inst_label_name   = (boost::format("%s/%06d.label") % est_label_dir_ % i).str();
  string ground_label_name = (boost::format("%s/%06d.label") % ground_label_dir_ % i).str();
...
void HeLiPRLoader::assignLabels(const std::vector<uint32_t> ground_labels,
                                const std::vector<uint32_t> instance_labels,
                                pcl::PointCloud<pcl::PointXYZI> &src_cloud,
                                uint32_t &max_instance) {
  max_instance = 0;
