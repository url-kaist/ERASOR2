...
void parseStaticAndDynamic(const pcl::PointCloud<pcl::PointXYZI> &cloud,
  ...
  for (const auto &pt : cloud.points) {
    ...
    if (find(DYNAMIC_CLASSES.begin(), DYNAMIC_CLASSES.end(), semantic_label) ==
        DYNAMIC_CLASSES.end()) {
...
void radiusSearch(const pcl::PointCloud<pcl::PointXYZI> &query_cloud,
  ...
  for (auto &query_pcl : query_cloud.points) {
    ...
    int num_matched = kdtree.radiusSearch(&query_pt[0], radius, ret_matches);
...
void radiusSearchWithAdaptiveRadii(const pcl::PointCloud<pcl::PointXYZI> &query_cloud,
  ...
  for (auto &query_pcl : query_cloud.points) {
    ...
    int num_matched = kdtree.radiusSearch(&query_pt[0], radii[query_idx], ret_matches);
...
void voxelize_preserving_labels(pcl::PointCloud<pcl::PointXYZI>::Ptr src,
   ...
   * so this function is to conduct voxelization followed by nearest points search to re-assign the
   * label of each point */
...
void voxelize_preserving_labels_by_nanoflann(pcl::PointCloud<pcl::PointXYZI>::Ptr src,
  ...
  voxel_filter.filter(*ptr_voxelized);
  if (verbose)
    cout << "\033[1;35m[Voxelization] " << src->points.size() << " => "
         << ptr_voxelized->points.size() << "\n";
  ...
  if (verbose) {
    cout << "Elapsed time in milliseconds: "
         << chrono::duration_cast<chrono::milliseconds>(t_f_e - t_v_s).count() << " ms("
         << chrono::duration_cast<chrono::milliseconds>(t_v_e - t_v_s).count() << " ms + ";
