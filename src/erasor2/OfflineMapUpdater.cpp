...
OfflineMapUpdater::OfflineMapUpdater() {
  sub_node_ = nh.subscribe<erasor::node>(
      "/node/combined/optimized", 2000, &OfflineMapUpdater::callback_node, this);
  sub_flag_ =
      nh.subscribe<std_msgs::Float32>("/saveflag", 10, &OfflineMapUpdater::callback_flag, this);
  ...
  pub_debug_query_egocentric_ =
      nh.advertise<sensor_msgs::PointCloud2>("/MapUpdater/debug/pc_curr_body", 100);
  pub_debug_map_egocentric_ =
      nh.advertise<sensor_msgs::PointCloud2>("/MapUpdater/debug/map_body", 100);
  pub_debug_map_arranged_init_ =
      nh.advertise<sensor_msgs::PointCloud2>("/MapUpdater/debug/map_init_arranged", 100);
...
void OfflineMapUpdater::load_global_map() {
  ...
  cout << "[MapUpdater] Path: " << map_name_ << endl;
...
void OfflineMapUpdater::save_static_map(float voxel_size) {
  ...
  erasor_utils::voxelize_preserving_labels(
      ptr_src, map_to_be_saved, voxel_size);  // 0.05m is the criteria!
  // 2. Save the cloudmap
...
void OfflineMapUpdater::callback_node(const erasor::node::ConstPtr &msg) {
  ...
  if (stack_count % removal_interval_ == 0) {
     ...
     *    So the coordinate is tranformed from lidar coord. to body coord. (in fact, it depends on
     * your own env.)
     * */
    ...
    *query_voi_ = *ptr_query_body;
    ...
    //
    //        ROS_INFO_STREAM("\033[1;32m" << setw(22) << "Extracting VoI takes " << end_voi -
    //        start_voi << "s\033[0m");
    //
    ...
    //
    //        /**< 3. Conduct Scan Ratio Test & set map_static_estimate and
    //        map_egocentric_complement
    ...
    //
    //        ROS_INFO_STREAM("\033[1;32m" << setw(22) << "ERASOR takes " << middle - start <<
    //        "s\033[0m");
    //
    ...
    //
    //        erasor_utils::parseStaticAndDynamic(*map_arranged_, *dynamic_objs_to_viz_,
    //        *static_objs_to_viz_);
    //
    //        /*** Just for debugging */
    ...
    //            erasor_utils::voxelize_preserving_labels(ptr_src, *map_arranged_,
    //            map_voxel_size_); auto end_voxel = ros::Time::now().toSec(); map_arranged_->width
    //            = map_arranged_->points.size(); map_arranged_->height = 1;
    ////            ROS_INFO_STREAM("Global voxelization operated: " << setw(10) << num_origin << "
    ///-> " << map_arranged_->points.size());
    //            ROS_INFO_STREAM(setw(22) <<"Global voxel. takes " << end_voxel - start_voxel <<
    //            "s");
    //        }
    if (environment_ != "outdoor") {
...
void OfflineMapUpdater::reassign_submap(double pose_x, double pose_y) {
  if (is_submap_not_initialized_) {
    set_submap(*map_arranged_global_,
               *map_arranged_,
               *map_arranged_complement_,
               pose_x,
               pose_y,
               submap_size_);
...
void OfflineMapUpdater::fetch_VoI(double x_criterion,
                                  double y_criterion,
  ...
  if (mode == "naive") {
    ...
    searchPoint.z = 0.5;
    std::cout << "\033[1;32mKDTREE mode " << (*map_arranged_).points.size() << "\033[0m"
              << std::endl;
    ...
    *cloud = *map_arranged_;
    ...
    if (kdtree.radiusSearch(
            searchPoint, max_range_ + 0.5, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0) {
      // To get outlier
...
void OfflineMapUpdater::body2origin(const pcl::PointCloud<pcl::PointXYZI> src,
                                    pcl::PointCloud<pcl::PointXYZI> &dst) {
  pcl::PointCloud<pcl::PointXYZI>::Ptr ptr_src(new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr ptr_transformed(new pcl::PointCloud<pcl::PointXYZI>);
  *ptr_src = src;
...
void OfflineMapUpdater::print_status() {
  ...
  ROS_INFO_STREAM(map_static_estimate_->points.size()
                  << " + " << map_egocentric_complement_->points.size() << " - "
                  << map_rejected_->points.size());
  ROS_INFO_STREAM(" = \033[1;33m" << map_static_estimate_->points.size() +
                                         map_egocentric_complement_->points.size() -
                                         map_rejected_->points.size()
                                  << "\033[0m");
  ROS_INFO_STREAM(map_arranged_->points.size()
                  << " " << map_filtered_->points.size() << " \033[4;32m"
                  << map_outskirts_->points.size() << "\033[0m");

  std::cout << "[Debug] Total: " << map_arranged_->points.size() << std::endl;
  std::cout << "[Debug] " << (double)dynamic_objs_to_viz_->points.size() / num_pcs_init_ * 100
            << setw(7) << " % <- " << dynamic_objs_to_viz_->points.size() << " / " << num_pcs_init_
            << std::endl;
  std::cout << "[Debug] " << (double)static_objs_to_viz_->points.size() / num_pcs_init_ * 100
            << setw(7) << "% <- " << static_objs_to_viz_->points.size() << " / " << num_pcs_init_
            << std::endl;
}

void OfflineMapUpdater::set_path(nav_msgs::Path &path,
                                 std::string mode,
                                 const erasor::node &node,
...
void OfflineMapUpdater::publish(const sensor_msgs::PointCloud2 &map,
...
void OfflineMapUpdater::publish(const pcl::PointCloud<pcl::PointXYZI> &map,
                                const ros::Publisher &publisher) {
