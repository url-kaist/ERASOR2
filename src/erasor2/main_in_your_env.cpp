...
void assignLabels(const std::vector<uint32_t> ground_labels, const std::vector<uint32_t> instance_labels,
                  ...
                  pcl::PointCloud<PointType>& src_cloud, uint32_t& max_instance) {
    max_instance = 0;
...
void voi2xygrid(
        ...
        float max_height, float min_height, float range, float resolution,
        vector<pcl::PointCloud<PointType>> &xygrid,
        pcl::PointCloud<PointType> &complement, std::string format="gridmap") {
    const int width = static_cast<int>(2 * range / resolution);
    const int height = static_cast<int>(2 * range / resolution);
    xygrid.resize(width * height);
...
//    erasor::OfflineMapUpdater updater = erasor::OfflineMapUpdater();
    ...
    ros::Publisher DynMapPublisher = nh.advertise<sensor_msgs::PointCloud2>("/erasor2/dyn_points_all", 100, true);
    ros::Publisher DynCurrScanPublisher = nh.advertise<sensor_msgs::PointCloud2>("/erasor2/dyn_points", 100, true);
    ros::Publisher CurrScanPublisher = nh.advertise<sensor_msgs::PointCloud2>("/erasor2/curr_scan", 100, true);
    ros::Publisher EgocentricGridPublisher = nh.advertise<grid_map_msgs::GridMap>("/erasor2/gridmap_egocentric", 100, true);
    ros::Publisher GridPublisher = nh.advertise<grid_map_msgs::GridMap>("/erasor2/gridmap", 100, true);
    ros::Publisher VizMarkerPublisher = nh.advertise<visualization_msgs::Marker>("/erasor2/target_grid_loc", 100, true);
    ...
    for (int i = INIT_IDX; i < END_IDX; ++i) {
        ...
        assignLabels(ground_labels, instance_labels, min_z_voi, max_z_voi,
                     *src_cloud, max_instance);
        ...
        voi2xygrid(*src_cloud, 0.0, 0.0, 0.0, max_z_voi, min_z_voi,
                   range_of_interest, egocentric_grid_resolution,
                   xygrid, complement);
        grid_map::GridMap gridmap = setEgocentricGridMap(range_of_interest, egocentric_grid_resolution, xygrid);
        // Viz
    ...
    for (int k = 0; k < num_data; ++k) {
        ...
        pos_approx(0) = grid_map_info.x_length / 2 + grid_map_info.center_x - w_pc * grid_map_info.resolution;
        pos_approx(1) = grid_map_info.y_length / 2 + grid_map_info.center_y - h_pc * grid_map_info.resolution;
...
//        std::cout << pos_x_approx << " , " << pos_y_approx << std::endl;
        voi2xygrid(pcs[k], pos_approx(0), pos_approx(1), poses_submap[k](2, 3), max_z_voi, min_z_voi,
                   range_of_interest, grid_resolution,
                   xygrids[k], complement);
        vector<pcl::PointCloud<PointType>> map_grid;
        pcl::PointCloud<PointType>::Ptr dummy(new pcl::PointCloud<PointType>);
        voi2xygrid(*map_partial, pos_approx(0), pos_approx(1), poses_submap[k](2, 3), max_z_voi, min_z_voi,
                   range_of_interest, grid_resolution,
                   map_grid, *dummy);
    ...
    for (int k = 0; k < num_data; ++k) {
        ...
        for (const auto& pt: each_pc) {
           if ( std::find(ids.begin(), ids.end(), pt.intensity) != ids.end()) {
               if (pt.intensity == NOT_VOLUME_OF_INTEREST) { continue; }
