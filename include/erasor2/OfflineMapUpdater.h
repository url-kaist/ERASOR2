...
class OfflineMapUpdater {
 private:
  ...
  Eigen::Matrix4f
      tf_lidar2body_; /**< Transformation matrix between the initial pose to the origin */
  ...
  void body2origin(const pcl::PointCloud<pcl::PointXYZI> src, pcl::PointCloud<pcl::PointXYZI> &dst);
  ...
  void fetch_VoI(double x_criterion,
                 double y_criterion,
  ...
  void publish(const sensor_msgs::PointCloud2 &map, const ros::Publisher &publisher);
