...
class VOXEL_LOC {
 public:
  ...
  explicit VOXEL_LOC(const int64_t vx = 0, const int64_t vy = 0, const int64_t vz = 0) {
    vloc[0] = vx;
    vloc[1] = vy;
    vloc[2] = vz;
  }
...
class Voxel {
 public:
  ...
  explicit Voxel(VOXEL_LOC<PointT> _pos, int _adj = 2, int _ver = 5) {
    pos = _pos;
...
class HashVoxelGrid {
 ...
 public:
   ...
   */
  inline unsigned int getMinimumPointsNumberPerVoxel() const { return min_points_per_voxel_; }
  ...
  inline void filter(PointCloud& output) {
    ...
    for (const auto& point : input_.points) {
      double loc_xyz[3] = {
          point.x / leaf_size_[0], point.y / leaf_size_[1], point.z / leaf_size_[2]};
  ...
  //        }
  //
  //        cout << "remaining num of voxels_: " << voxels_.size() << " (min # of points: " <<
