 ...
 * Estimation](http://mars.cs.umn.edu/tr/reports/Trawny05b.pdf). In the implementation, we have 4
 * statements so that we avoid a division by zero and instead always divide by the largest diagonal
 * element. This all comes from the definition of a rotation matrix, using the diagonal elements and
 * an off-diagonal. \f{align*}{ \mathbf{R}(\bar{q})= \begin{bmatrix}
