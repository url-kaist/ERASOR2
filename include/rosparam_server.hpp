...
class RosParamServer {
 public:
  ...
  RosParamServer() {
    ...
    nh_.param<bool>(
        "/dataloader/distinguish_temporal_trajectories", distinguish_temporal_trajectories_, false);
    ...
    nh_.param<bool>("/pose_corrector/correct_poses_by_submap_matching",
                    correct_poses_by_submap_matching_,
                    false);
    ...
    nh_.param<float>(
        "/erasor2/moving_object_detection/negative_log_odds", negative_log_odds_, -2.0);
    nh_.param<float>(
        "/erasor2/moving_object_detection/obj_score_soft_thr", obj_score_soft_thr_, 0.8);
    nh_.param<float>(
        "/erasor2/moving_object_detection/obj_score_hard_thr", obj_score_hard_thr_, 20.0);
    ...
    nh_.param<float>(
        "/erasor2/over_segmentation/ratio_of_unknown_prior", ratio_of_unknown_prior_, 0.25);

    nh_.param<int>("/erasor2/volumetric_outlier_removal/window_size", window_size_, 1);
    nh_.param<bool>("/erasor2/volumetric_outlier_removal/use_adaptive_voxel_size",
                    use_adaptive_voxel_size_,
                    false);
    nh_.param<float>(
        "/erasor2/volumetric_outlier_removal/vor_cand_score_thr", vor_cand_score_thr_, 3.0);
    ...
    ext_rot_ =
        Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(ext_rot_vec_.data(), 3, 3);
    ext_trans_ = Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(
        ext_trans_vec_.data(), 3, 1);
    ...
    NoiseMapPublisher =
        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/noise_points_all", 100, true);

    CurrCloudPublisher = nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/curr_scan", 100, true);
    DynCurrCloudPublisher =
        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/dyn_points", 100, true);
    DynInstCurrCloudPublisher =
        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/dyn_inst_points", 100, true);
    RejectedDynCurrCloudPublisher =
        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/rejected_dyn_points", 100, true);
    OutlierCurrCloudPublisher =
        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/outlier_points", 100, true);
    AcceptedMovingObjScorePublisher = nh_.advertise<visualization_msgs::MarkerArray>(
        "/erasor2/accetped_moving_obj_scores", 100, true);
    RejectedMovingObjScorePublisher = nh_.advertise<visualization_msgs::MarkerArray>(
        "/erasor2/rejected_moving_obj_scores", 100, true);

    NonGroundCurrCloudPublisher =
        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/non_ground_points", 100, true);
    GroundCurrCloudPublisher =
        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/ground_points", 100, true);
    NoiseCurrCloudPublisher =
        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/noise_points", 100, true);
    ...
    EgocentricGridPublisher =
        nh_.advertise<grid_map_msgs::GridMap>("/erasor2/gridmap_egocentric", 100, true);
    GridPublisher = nh_.advertise<grid_map_msgs::GridMap>("/erasor2/gridmap", 100, true);
    AdaptiveRangePublisher =
        nh_.advertise<visualization_msgs::Marker>("/erasor2/adaptive_range", 100, true);

    PosePublisher = nh_.advertise<geometry_msgs::PoseStamped>("/erasor2/pose", 100, true);

    //        pub_map_rejected  = nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/map_rejected",
    //        100); pub_curr_rejected =
    //        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/curr_rejected", 100); pub_map_init
    //        = nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/map_init", 100); pub_curr_init =
    //        nh_.advertise<sensor_msgs::PointCloud2>("/erasor2/curr_init", 100);
    //
    //        pub_map_marker     =
    //        nh_.advertise<jsk_recognition_msgs::PolygonArray>("/erasor2/map_marker", 100);
    //        pub_curr_marker    =
    //        nh_.advertise<jsk_recognition_msgs::PolygonArray>("/erasor2/curr_marker", 100);
    //        pub_viz_bin_marker =
    //        nh_.advertise<jsk_recognition_msgs::PolygonArray>("/erasor2/polygons_marker", 100);
    //        pub_h_marker  =
    //        nh_.advertise<visualization_msgs::MarkerArray>("/pseudo_occupancy_array", 100, true);
    //
